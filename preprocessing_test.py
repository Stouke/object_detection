...
fusion_model = create_fusion_model((416, 416, 1))
fusion_model.load_weights('C:/Users/n2309064h/Desktop/Multimodal_code/object_detector_(yolov3_w_fusion)/preprocessed_data/trained_fusion_model.h5')

# Assign labels based on detected objects and fusion model predictions
def assign_label(class_ids, fusion_prediction):
    # Example criteria: 
    # If a car is detected by YOLOv3, label as 1
    # If a pedestrian is detected by YOLOv3, label as 2
    # If the fusion model predicts positive for pedestrian, label as 3
    for class_id in class_ids:
        if classes[class_id] == 'car':
            return 1
        elif classes[class_id] == 'pedestrian':
            return 2
    if fusion_prediction > 0.5:
        return 3
    return 0
